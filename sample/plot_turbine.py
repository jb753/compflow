...
f, a = plt.subplots()
Q2b = (
    cf.mcpTo_APo_from_Ma(Ma1_in,ga_in) / AR_in
...
def solve_newton(Ma1, ga, AR, Yp):
    ...
    if (cf.mcpTo_APo_from_Ma(1.,ga)
            < K * (1.+Yp*(1.-1./cf.Po_P_from_Ma(0.,ga)))):
        return np.nan
    print('beans')

    def f(x):
        return (
                K * (1.+Yp*(1.-1./cf.Po_P_from_Ma(x,ga)))
                - cf.mcpTo_APo_from_Ma(x,ga)
                )

    def fp(x):
        return (
                K * Yp * cf.der_Po_P_from_Ma(x,ga) / cf.Po_P_from_Ma(x,ga)**2.
                - cf.der_mcpTo_APo_from_Ma(x,ga)
                )
...
def solve_fixed_point(Ma1, ga, AR, Yp):
    ...
    for n in range(Niter-1):
        Ma_guess[n+1] = (
            K * (1.+Yp*(1.-1./cf.Po_P_from_Ma(Ma_guess[n],ga)))
            /  cf.mcpTo_APo_from_Ma(Ma_guess[n],ga) * Ma_guess[n]
            )
        if np.abs(Ma_guess[n+1]-Ma_guess[n])<Ma_tol:
...
T_fixed = timeit.Timer('solve_fixed_point(Ma1_in, ga_in, AR_in, Yp_in)',
        'from __main__ import solve_fixed_point,Ma1_in, ga_in, AR_in, Yp_in')
T_newton = timeit.Timer('solve_newton(Ma1_in, ga_in, AR_in, Yp_in)',
